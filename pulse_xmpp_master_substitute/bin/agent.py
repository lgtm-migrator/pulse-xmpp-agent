...
class MUCBot(slixmpp.ClientXMPP):
    ...
    def Clean_old_queue(self, nbsecond):
        """
            delete queue superieur a nb secondes
        """
        fichiers = [f for f in os.listdir("/dev/mqueue") if f != "mysend" and os.path.isfile(os.path.join("/dev/mqueue", f))]
        for f in fichiers:
            pathf = os.path.join("/dev/mqueue", f)
            if (time.time() - os.path.getmtime(pathf) > nbsecond):
                try:
    ...
    def clean_my_mpqueue(self):
        """
            delete all queue
        """
        fichiers = [f for f in listdir("/dev/mqueue")
                    if isfile(join("/dev/mqueue", f))]
        for f in fichiers:
            if f != 'mysend':
